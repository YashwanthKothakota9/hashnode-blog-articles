...
* Now install `express`
    ...
    In earlier versions we need to explictly install types for express `@types/express`, from the `version:4.17.0` it is not needed as `express` includes its own `typescript` `definition` files directly within its `package`.
